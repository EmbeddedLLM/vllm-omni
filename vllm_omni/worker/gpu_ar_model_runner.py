...
class GPUARModelRunner(OmniGPUModelRunner):
    ...
    def sample_tokens(
        ...
        model_output = getattr(self, "_omni_last_model_output", None)
        self._omni_last_model_output = None
        multimodal_outputs = (
            getattr(model_output, "multimodal_outputs", None) if model_output is not None else None
        )
        text_hidden_states = (
            getattr(model_output, "text_hidden_states", None) if model_output is not None else None
        )
        ...
        num_scheduled_tokens_np = getattr(
            self, "_omni_num_scheduled_tokens_np", None
        )
                        ...
                        if isinstance(v, torch.Tensor) and v.shape[0] == hidden_states_cpu.shape[0]:
                            ...
                            for sk, sv in v.items():
                                if isinstance(sv, torch.Tensor) and sv.shape[0] == hidden_states_cpu.shape[0]:
                                    sub_dict[str(sk)] = (
                                        sv.detach().to("cpu")[start:end].contiguous()
                                    )
                            if sub_dict:
